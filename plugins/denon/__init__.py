...
class Denon(lib.connection.Client):
    ...
    def __init__(self, smarthome, host, port=23, cycle=10):
        ...
        self._status_objects = ['MV?', 'SI?', 'MU?', 'PSBAS ?', 'PSTRE ?']
        ...
        self._sh.scheduler.add('denon-status-update', self._update_status, cycle=cycle)
        self._sh.scheduler.change('denon-status-update', active=False)
    ...
    def found_terminator(self, data):
        ...
        if data == 'PWON':
            ...
            sleep(1.5)
            self._sh.scheduler.change('denon-status-update', active=True)
        # AVR entered standby
        elif data == 'PWSTANDBY':
    ...
    def _pollXML(self):
        ...
        if resp.status != 200:
            logger.error("Denon: XML Request failed")
            return None

        resp = resp.read().decode("utf-8")
        status = xml.dom.minidom.parseString(resp)

        # extract power state
        power = status.getElementsByTagName('Power')[0].getElementsByTagName('value')[0].childNodes[0].nodeValue
        self._items['power']((True if power.upper() == 'ON' else False), 'Denon', self._host)

        # extract mute state
        mute = status.getElementsByTagName('Mute')[0].getElementsByTagName('value')[0].childNodes[0].nodeValue
        self._items['mute']((True if mute.upper() == 'ON' else False), 'Denon', self._host)

        # extract volume state (in dB)
        vol = status.getElementsByTagName('MasterVolume')[0].getElementsByTagName('value')[0].childNodes[0].nodeValue
        vol = int(float(vol) + 80)
        self._items['volume'](vol, 'Denon', self._host)
